...
export class FirebaseuiAngularLibraryService {
  ...
  constructor(angularFireAuth: AngularFireAuth) {
    ...
    if (!(<any>window).firebaseUiEnInstance) {
      (<any>window).firebaseUiEnInstance = new firebaseui.auth.AuthUI(angularFireAuth.auth);
    }
    this.firebaseUiInstance = (<any>window).firebaseUiEnInstance as firebaseui.auth.AuthUI;
  }
}
